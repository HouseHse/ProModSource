...
new Handle:SM_hBufferItems;
new Handle:SM_hStaticPillValue;
new Handle:SM_hStaticAdrenValue;
new Handle:SM_hStaticKitValue;
...
public OnPluginStart()
{
	...
	SM_hShowMaxDist = CreateConVar("SM_showmaxdist", "1", "L4D2 Custom Scoring - Print custom max distances.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	SM_hBufferItems = CreateConVar("SM_bufferitems", "1", "L4D2 Custom Scoring - Should we add the items worth to total health before calculating bonus? (Default 1)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	SM_hStaticPillValue = CreateConVar("SM_pillpoints", "0", "L4D2 Custom Scoring - How many static points pills are worth.", FCVAR_PLUGIN, true, 0.0);

	SM_hStaticAdrenValue = CreateConVar("SM_adrenpoints", "0", "L4D2 Custom Scoring - How many static points adrenaline are worth.", FCVAR_PLUGIN, true, 0.0);

	SM_hStaticKitValue = CreateConVar("SM_kitpoints", "0", "L4D2 Custom Scoring - How many static points medkits are worth.", FCVAR_PLUGIN, true, 0.0);
...
stock SM_CalculateScore()
{
	...
	return RoundToFloor(fScore * SM_fMapMulti * SM_fHBRatio + 400 * SM_fMapMulti * SM_fSurvivalBonusRatio) * iAliveCount + iStaticHealth;
}
...
stock Float:SM_CalculateAvgHealth(&iAliveCount=0)
{
	...
	new bool:BufferItems = GetConVarBool();
	// Temporary Storage Variables for inventory
	new iTemp;
	new iCurrHealth;
	new iCurrTemp;
	new iIncapCount;
	new iStaticPillValue = GetConVarInt(SM_hStaticPillValue);
	new iStaticAdrenValue = GetConVarInt(SM_hStaticAdrenValue);
	new iStaticKitValue = GetConVarInt(SM_hStaticKitValue);
			...
			if (IsPlayerAlive(index))
			{
				...
				if (!SM_IsPlayerIncap(index))
				{
					...
					if (iTemp > -1)
					{
						...
						else if (StrEqual(strTemp, "weapon_adrenaline"))
						{
							if(BufferItems)
							{
	...
	iStaticHealth = (iPillCount * iStaticPillValue) + (iAdrenCount * iStaticAdrenValue) + (iKitCount * iStaticKitValue);
